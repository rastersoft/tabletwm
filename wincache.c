...
struct wincache_element *wincache_add_element(uint32_t window) {
	...
	if (element == NULL) { // that window id doesn't exists in the list
		...
		if (wincache_list != NULL) {
			wincache_list->prev = element;
		}
		element->next = wincache_list;
		wincache_list = element;
	}
	return (element);
}
...
struct wincache_element *wincache_fill_element(uint32_t window, uint32_t flags) {
	...
	xcb_get_property_cookie_t window_hints_cookie;
	...
	if (flags & WINCACHE_WINDOW_HINTS) {
		window_hints_cookie = xcb_get_property(conn,0,window,atoms[TWM_ATOM_WM_HINTS],XCB_ATOM_WM_HINTS,0,2);
	}
	if (flags & WINCACHE_GEOMETRY) {
	...
	if (flags & WINCACHE_WINDOW_HINTS) {
		window_hints = xcb_get_property_reply(conn,window_hints_cookie,0);
